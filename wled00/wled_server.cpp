...
static String msgProcessor(const String& var)
{
  if (var == "MSG") {
    ...
    {
      messageBody += F("<br><br><form action=/settings><button class=\"bt\" type=submit>Inapoi</button></form>"); //button to settings
    } else if (optt == 254)
    {
      messageBody += F("<br><br><button type=\"button\" class=\"bt\" onclick=\"B()\">Inapoi</button>");
    }
    return messageBody;
  }
  return String();
